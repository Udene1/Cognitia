...
def test_observer_reads_real_git_history(tmp_path: Path):
    ...
    observer = GitRepositoryObserver(repo)

    observations = observer.commits()
    ...
    assert observer.head() == observations[0].commit_id
...
def test_ingestor_records_commits_as_neutral_experiences(tmp_path: Path):
    ...
    assert ingestor.ingested_commit_ids() == frozenset(
        experience.context["commit_id"] for experience in first
    )


def test_observer_rejects_non_repository(tmp_path: Path):
    observer = GitRepositoryObserver(tmp_path / "missing")
