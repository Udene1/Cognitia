...
class ClaimIdentityMatcher:
    ...
    def score(self, left: ExtractedClaim, right: ExtractedClaim) -> tuple[float, tuple[str, ...]]:
        left_tokens = _tokens(left.proposition)
        right_tokens = _tokens(right.proposition)
        ...
        # Shared entities and relations are stronger identity signals than raw
        # lexical overlap. Temporal disagreement is an explicit penalty.
        score = 0.45 * overlap + 0.30 * entities + 0.20 * relations + 0.05 * temporal
        if entities == 0 and relations == 0:
            score *= 0.75
        basis = []
        if overlap >= 0.5: basis.append("shared_terms")
        if entities > 0: basis.append("shared_entities")
        if relations > 0: basis.append("shared_relations")
        if temporal > 0: basis.append("shared_temporal_marker")
...
def _canonical(claim: ExtractedClaim) -> str:
    ...
    tokens = " ".join(sorted(_tokens(claim.proposition)))
