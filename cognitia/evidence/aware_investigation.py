...
from ..discovery_prediction import Prediction
...
class EvidenceAwareInvestigator:
    ...
    def __init__(self, sources: Mapping[str, EnvironmentSource], *, max_workers: int = 4,
                 convergence: EvidenceConvergenceEngine | None = None,
                 experiment_selector: DiscriminatingExperimentSelector | None = None) -> None:
    ...
    def investigate(self, claim: Claim, tasks: Sequence[InvestigationTask], *,
                    predictions: Sequence[Prediction] = (), limit: int = 10) -> EvidenceInvestigationTrace:
        ...
        next_experiment = self.experiment_selector.select(tuple(predictions)) if predictions else None
        return EvidenceInvestigationTrace(claim, tuple(tasks), tuple(results), evidence, assessment, next_experiment)
    ...
    def _to_evidence(claim: Claim, results: Sequence[object]) -> tuple[EvidenceRecord, ...]:
        ...
        for result in results:
            for observation in getattr(result, "observations", ()):
                ...
                source = EvidenceSource(id=observation.source, kind=metadata.get("kind", "environment"),
                                        name=observation.source, reliability=observation.reliability,
                                        metadata=metadata)
                upstream = tuple(filter(None, metadata.get("upstream_ids", "").split(",")))
                records.append(EvidenceRecord(id=observation.id, claim_id=claim.id, source=source,
                    content=observation.content, supports=supports,
                    lineage=SourceLineage(observation.source, upstream), observation_id=observation.id,
                    measured_at=metadata.get("measured_at"), method=metadata.get("method"), metadata=metadata))
