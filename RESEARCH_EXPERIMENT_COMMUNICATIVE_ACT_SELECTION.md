...
**Experiment 1 implementation complete; controlled behavior CI-validated.**
...
The implementation deliberately does not use an LLM and does not generate prose as part of act selection. It tests the smallest current distinction between cognitive state and communicative action.
...
# Experiment 1 result — 2026-09-16

GitHub Actions run **35056983672** completed successfully for PR #8.

The communication-specific unit tests passed as part of the successful `test` job. The controlled state produced four distinct acts under four objectives:
...
The same run also passed the ambiguity and capability-limit cases and rejected an explicit candidate-to-established-fact upgrade.

### Result interpretation

**Supported:** the explicit representation can separate communicative objective from underlying cognitive state and select different communicative acts without increasing epistemic warrant in the tested cases.

**Not established:** that Cognitia learned this capability. The mapping is currently developer-specified and deterministic.

The result therefore advances the research from:

```text
hypothesis: communication may be a separate cognitive capability
```

to:

```text
controlled implementation: the distinction is executable and testable
```

but not to:

```text
claim: Cognitia has learned communication
```

The next experiment must therefore be discriminating rather than merely additive.
...
Experiment 1 has demonstrated only part of item 3 and item 5, plus controlled support for the representation needed to pursue the remaining items. It has not demonstrated the full early-success set.
...
Do not expand the communication taxonomy merely because Experiment 1 passed.

The next discriminating experiment should vary **interaction context/recipient** while holding the cognitive state and communicative objective controlled. This tests whether the selected communicative action depends on who the communication is for and what the interaction permits.
